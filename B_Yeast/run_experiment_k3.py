...
from Ob_Met_Net_solmethods import MILP_solve_NOP,CB_sol_2_NOP, Inner_check_vs_ys_NOP,Result_cb,Result
from Ob_Met_Net_Yeast import MetNet_Yeast
...
# =========================================== K=3 ===========================
try:
    m = MILP_solve_NOP(network=metnet,k=3,log=False,speed=True)
except:
    m = Result(metnet.Name,['NA' for i in range(ks)],[1 for i in metnet.M],metnet.FBA,'inf','NA') 

try:
    cb = CB_sol_2_NOP(network=metnet,k=3,log=False,speed=True)
except:
